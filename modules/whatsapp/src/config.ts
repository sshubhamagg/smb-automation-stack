...
function optionalLogLevel(name: string, defaultValue: Config['logLevel']): Config['logLevel'] {
  const raw = process.env[name];
  if (!raw) return defaultValue;
  const valid: Config['logLevel'][] = ['DEBUG', 'INFO', 'WARN', 'ERROR'];
  const upper = raw.toUpperCase() as Config['logLevel'];
...
function optionalStoreBackend(name: string, defaultValue: Config['storeBackend']): Config['storeBackend'] {
  const raw = process.env[name];
  if (!raw) return defaultValue;
