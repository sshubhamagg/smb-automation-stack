...
export class ValidationError extends Error {
  code: 'INVALID_PAYLOAD' | 'MISSING_FIELD' | 'INVALID_FORMAT';
  field?: string;
...
export interface ValidatedInbound {
  messageId: string;
  from: string;
  timestamp: string;
  type: string;
...
export function verifySignature(rawBody: Buffer | string, header: string, secret: string): boolean {
  try {
    ...
    const bodyBuffer = Buffer.isBuffer(rawBody) ? rawBody : Buffer.from(rawBody as string);
...
export function validateInbound(payload: unknown): ValidatedInbound | null {
  ...
  if (field !== 'messages') {
    throw new ValidationError(
      'INVALID_PAYLOAD',
      `Expected field='messages', got '${String(field)}'`,
      'field'
    );
  }
  ...
  if (messagingProduct !== 'whatsapp') {
    throw new ValidationError(
      'INVALID_PAYLOAD',
      `Expected messaging_product='whatsapp', got '${String(messagingProduct)}'`,
      'messaging_product'
    );
  }
...
export function validateOutbound(body: unknown): ValidatedOutbound {
  ...
  if (!isE164(recipient)) {
    throw new ValidationError(
      'INVALID_FORMAT',
      `recipient must be a valid E.164 phone number, got '${recipient}'`,
      'recipient'
    );
  }
