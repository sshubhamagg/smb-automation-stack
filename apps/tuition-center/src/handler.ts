...
import type {
  TuitionConfig,
  IncomingMessage,
} from "../../../flows/tuition-center/src/types";
...
function loadConfig(): TuitionConfig {
  ...
  return {
    ...
    aiProvider: (process.env["TUITION_AI_PROVIDER"] ?? "anthropic") as
      | "openai"
      | "anthropic"
      | "local"
      | "nvidia",
...
const modules: Modules = {
  storage: (input: unknown) =>
    storageExecute(input as Parameters<typeof storageExecute>[0]),
  communication: (input: unknown) =>
    communication.execute(input as { to: string; message: string }),
...
const TEACHER_PHONE = process.env["TUITION_TEACHER_PHONE"] ?? "";
async function send(to: string, message: string): Promise<void> {
  if (to.replace("+", "") !== TEACHER_PHONE.replace("+", "")) {
    await new Promise((resolve) => setTimeout(resolve, 5000)); // 3s delay for students
  }
  await communication.execute({ to, message }).catch((err: unknown) => {
    console.error(
      "[tuition] send failed:",
...
export async function handleTeacherMessage(
  ...
  if (!routing) {
    if (config.mode === "ai") {
      await send(
        phone,
        "Could not extract intent. Try: present +<phone>, paid +<phone> <amount>",
      );
    }
    return;
  }
  ...
  if (nextFlow === "query-attendance") {
    const ctx = buildQueryAttendanceCtx(
      { phone_number: phone },
      parsed,
      config,
    );
...
export async function handleFeeInit(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[tuition] handleFeeInit config error:",
      err instanceof Error ? err.message : err,
    );
    return;
  }
  ...
  console.log(
    `[tuition] fee-init: ${students.length} students found for ${month}`,
  );

  let written = 0;
  for (const student of students) {
    if ((student["Status"] ?? "").trim().toUpperCase() !== "ACTIVE") continue;
    ...
    if (!writeResult.ok) {
      console.error(
        `[tuition] fee-init write failed for ${studentId}:`,
        writeResult.error,
      );
    } else {
      written++;
...
export async function handleFeeReminders(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[tuition] handleFeeReminders config error:",
      err instanceof Error ? err.message : err,
    );
    return;
  }
